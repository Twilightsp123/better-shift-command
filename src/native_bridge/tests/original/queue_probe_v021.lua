out("[BRIDGE_OBSERVER_V021] ENTER")

-- Client patch for the byte-locked 0.1.1-observer-floatabi DLL.
-- Does not implement command provenance, a revision gate, or unit control.
-- Exact uint32 values remain decimal STRINGS, including epoch and cursor.
local TAG = "[BRIDGE_OBSERVER_V021] "
local CLIENT_KEY = "__WH3_OBSERVER_V021_CLIENT"
local TIMER_KEY = "WH3ObserverV021Journal"
local PAGE_SIZE, MAX_PAGES_PER_TICK = 64, 8
local b = rawget(_G, "bm")

local function log(message)
    local line = TAG .. tostring(message)
    local f = rawget(_G, "out")
    if type(f) == "function" then
        local ok = pcall(f, line)
        if ok then return end
    end
    if b and type(b.out) == "function" then
        local ok = pcall(b.out, b, line)
        if ok then return end
    end
    if type(print) == "function" then pcall(print, line) end
end

local previous = rawget(_G, CLIENT_KEY)
...
local path = rawget(_G, "WH3_NATIVE_BRIDGE_DLL") or ".\\wh3_native_bridge.dll"
...
for _, name in ipairs({"version", "number_abi_probe", "exact_id_probe", "capabilities",
    "get_status", "start_observer", "begin_battle", "end_battle", "read_journal"}) do
    if type(bridge[name]) ~= "function" then log("FAIL MISSING_API " .. name); return end
end

local ok, version = pcall(bridge.version)
if not ok or version ~= "0.1.1-observer-floatabi" then
...
local function caps_read()
    ...
    -- This client is deliberately bound to the observer-only baseline.
    if caps.exact_source ~= false or caps.verified_issue ~= false
        or caps.controller_connected ~= false or caps.complete_command_batch ~= false then
...
local function poll()
    ...
    if not success then fail("POLL_EXCEPTION " .. clean(err)) end
end
...
local function start_session()
    ...
    local success, epoch, err = pcall(bridge.begin_battle, "wh3_observer_v021_" .. tostring(b))
...
log("HOOKS_ACTIVE observer_hooks_installed=true exact_source=false verified_issue=false")
rawset(_G, "wh3_bridge", bridge)
...
if not state.failed and not state.stopped then log("CLIENT_READY_WAITING_FOR_NATIVE_RECORDS") end
