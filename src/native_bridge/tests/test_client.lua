...
local function fixture(opts)
    ...
    function bridge.capabilities()
        ...
        return {release_approved=false,host_lua_number='float32',host_lua_number_bytes=4,
    ...
    function bridge.acknowledge(epoch,cursor)
        eq(epoch,f.epoch);eq(type(cursor),'string');f.calls.ack=(f.calls.ack or 0)+1
        if opts.ack_failure then return nil,'ACK_DENIED' end
        return true
    end
    ...
    function f.state()return env.__WH3_OBSERVER_V041_CLIENT end
...
test('old production client: one-string begin success leads to nil epoch and zero reads',function()
    local f=fixture({version='0.1.1-observer-floatabi'}); f.put(2);f.load(ROOT..'/tests/original/queue_probe_v02.lua');f.phase('Deployment');f.tick()
...
test('old client isolated second bug: force two-return begin fixture exposes string-number comparison',function()
    local f=fixture({old_fake_two_returns=true,version="0.1.1-observer-floatabi"});f.put(1)
    f.load(ROOT..'/tests/original/queue_probe_v02.lua');f.phase('Deployment')
...
test('v0.2.1 rawget regression: inherited WH3 globals leave only ENTER and no observer start',function()
    ...
    f.load(ROOT..'/tests/original/queue_probe_v021.lua')
...
test('new battle manager closes old client',function()
    local a=fixture().active();local b=fixture({epoch='2'});b.env.__WH3_OBSERVER_V041_CLIENT=a.state();b.active()
...
test('mapped-source capability is allowed but never auto-arms commands',function()local f=fixture({exact_source=true}).active();eq(f.calls.commands,0);assert(not f.state().failed)end)
...
test('acknowledgement follows validated journal consumption',function()
 local f=fixture();f.put(150);f.active();eq(f.calls.ack,3);eq(f.state().cursor,'150')
end)
test('acknowledgement failure stops the session without retrying',function()
 local f=fixture({ack_failure=true});f.put(1);f.active();eq(f.calls.ack,1);eq(f.calls.finish,1);assert(has(f.logs,'ACKNOWLEDGE ACK_DENIED'))
end)
