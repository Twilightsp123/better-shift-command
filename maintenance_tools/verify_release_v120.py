...
def main():
    ...
    if raw!=expected: raise SystemExit('FAIL: pack is not deterministic v1.2.0 build')
    ...
    for m in (b'local CONTROLLER_VERSION = "1.2.0"',b'local RUN_ID = "V1_2_0"',b'BETTER_SHIFT_COMMAND_V1.2.0',b'local DEBUG_TELEMETRY = false',b'1.0.15-r4-evidence-v3-validated-userdata-root'):
        if m not in c: raise SystemExit('FAIL: missing v1.2.0 release marker '+repr(m))
    print('PASS: deterministic Better Shift Command v1.2.0 release pack')
