-- Turn Handoff P2B-HF5 / 0.2.5. Attack-first extension on the HF4.1 runtime baseline.
...
local TAG = "[TURN_HANDOFF_P1E_P2B] "
out(TAG .. "ENTER version=" .. CONTROLLER_VERSION .. " run=" .. RUN_ID .. " test_profile=" .. TEST_PROFILE .. " phase=" .. CONTROLLER_PHASE .. " build=P2B-HF5")
...
local function observe_attack(st,now)
    ...
    log("ATTACK_POST_SAMPLE uid="..st.uid.." gen="..st.gen.." issue="..t.issue..
...
local function observe_cold_idle(st,now)
    ...
    if good then
        ...
        if not st.cold_idle or st.cold_idle.native_state~=native_state then
            log("COLD_IDLE_READY uid="..st.uid.." rev="..rev.." native_state="..native_state.." model_ms="..string.format("%.0f",now)..
        ...
        if st.cold_idle then log("COLD_IDLE_REVOKED uid="..st.uid.." model_ms="..string.format("%.0f",now)) end
        st.cold_idle=nil
        ...
        if st.cold_note~=note then
            st.cold_note=note
            log("COLD_IDLE_UNAVAILABLE uid="..st.uid.." native_state="..clean(native_state)..
...
local function observe_exit(st,now)
    ...
    log("P2_EXIT_OBSERVED uid="..st.uid.." gen="..st.gen.." issue="..t.issue..
    ...
    if remaining and remaining<=6 then
        log("P2_EXIT_REACHED_PN uid="..st.uid.." gen="..st.gen.." issue="..t.issue..
            " attack_issue="..t.attack_issue.." model_ms="..string.format("%.0f",now)..
...
local function ingest(r,now)
    ...
    log("ORDER uid="..st.uid.." serial="..r.serial.." type="..clean(r.order_type).." status="..clean(r.status)..
        ...
        if p.kind=="MOVE" then
            ...
            if not st.plan[p.idx+1] then
                ...
                if st.moves>0 then
                    ...
                    log("CASE_ROUTE_PASS uid="..st.uid.." gen="..st.gen.." own_moves="..st.moves.." attack_target="..p.target_uid.." visual_smoothness=UNASSESSED")
                end
    ...
    elseif #st.actions==0 then
        ...
        if not good then
            st.blocked=true
            log("QUEUED_WITHOUT_REPLACE_IGNORED uid="..st.uid.." serial="..r.serial..
        ...
        log("COLD_IDLE_SEED uid="..st.uid.." gen="..st.gen.." serial="..a.serial..
    ...
    log("ACTION_CAPTURE uid="..st.uid.." gen="..st.gen.." idx="..#st.actions.." type="..a.type.." serial="..a.serial.." queued="..tostring(a.queued).." seed_mode="..(a.seed_mode or "REPLACE_OR_APPEND").." model_ms="..string.format("%.0f",now)..
        (a.pos and (" x="..string.format("%.6f",a.pos.x).." z="..string.format("%.6f",a.pos.z)) or (" target="..clean(a.target_uid))))
    if active then
        log("PLAN_APPENDED uid="..st.uid.." gen="..st.gen.." serial="..a.serial..
...
local function dispatch(st,index,reason,g,now)
    ...
    if a.type=="ATTACK" then ATTACK_TRACE[st.uid]={gen=generation,issue=issue,started=now} end
...
local function advance_attack(st,now)
    ...
    local bbox_distance=unit_distance_to(st.unit,a.target)
    local center_distance=(st.pos and target_pos) and dist(st.pos,target_pos) or nil
    ...
    local actor_speed=median(st.speeds) or 0
    local instant_speed=st.speeds[#st.speeds] or 0
    local elapsed=math.max(0,now-t.accepted_ms)

    -- Log EACH interval: the collector reconstructs eligibility instead of trusting HOLD_DONE.
    log("ATTACK_TARGET_OBSERVED uid="..st.uid.." gen="..st.gen.." issue="..t.issue.." idx="..st.idx..
    ...
    if eligible and not t.previous_eligible then
        ...
        if not t.cancel_test_announced then
            log("ATTACK_RMB_CANCEL_TEST_READY uid="..st.uid.." gen="..st.gen.." issue="..t.issue..
...
local function advance(st,now)
    ...
    if not nexta then
        ...
        if remaining<=CFG.proximity and not st.tail_reached then
            ...
            log("FINAL_MOVE_POSITION_REACHED uid="..st.uid.." gen="..st.gen.." idx="..st.idx..
    ...
    if nexta.type=="ATTACK" then
        ...
        if not reason and now-st.last_wait>=1000 then
            st.last_wait=now
            log("ATTACK_TRANSITION_WAIT uid="..st.uid.." gen="..st.gen.." remaining="..string.format("%.6f",g.remaining)..
...
local function poll_core()
    ...
    if now-S.last_heartbeat>=2000 then
        S.last_heartbeat=now
        log("HEARTBEAT run="..RUN_ID.." model_ms="..string.format("%.0f",now).." cursor="..S.cursor..
    ...
    for _,c in ipairs(S.cancel_checks) do
        if not c.logged and now-c.start>=CFG.cancel_observe_ms then
            ...
            log("CASE_CANCEL_PASS uid="..c.uid.." gen="..c.gen.." observation_ms="..string.format("%.0f",now-c.start).." claim=NO_NEW_LUA_DISPATCH_FOR_CANCELLED_GENERATION")
        end
    end
end
local function poll()
