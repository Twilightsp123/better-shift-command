...
-- Native dependency: v0.5.1 per-kind calibration Bridge.
...
local CONTROLLER_VERSION = "0.2.6" -- v1.0.1 release: per-kind bridge calibration + native Move canonicalization tolerance.
...
out(TAG .. "ENTER version=" .. CONTROLLER_VERSION .. " run=" .. RUN_ID .. " test_profile=" .. TEST_PROFILE .. " phase=" .. CONTROLLER_PHASE .. " build=BETTER_SHIFT_COMMAND_V1.0.1 debug_telemetry=" .. tostring(DEBUG_TELEMETRY))
...
local function arm(kind)
    ...
    dlog("BRIDGE_ARMED kind="..kind.." production_exact_source=false accepted_move="..tostring(s.accepted_move_seen==true).." accepted_attack="..tostring(s.accepted_attack_seen==true))
    return true
end
...
local function collections()
    ...
    for i=1,own:count() do
        ...
        if not S.states[u] then
            S.states[u]=new_state(su); dlog("REGISTER_LOCAL uid="..u)
        end
    end
    if enemy then
...
cancel=function(st,reason,now,observe)
    ...
    if st.plan or st.owned then
        ...
        dlog("GEN_CANCEL uid="..st.uid.." gen="..old.." reason="..reason.." owned="..tostring(st.owned).." tracked="..tostring(tracked).." terminal="..tostring(st.terminal).." phase="..clean(st.phase).." model_ms="..string.format("%.0f",now))
...
local function ingest(r,now)
    ...
    if r.source=="OUR_CONTROLLER" then
        ...
        if st.gen~=p.gen then
            dlog("LATE_OWN_ACK_IGNORED uid="..st.uid.." gen="..p.gen.." issue="..p.issue.." status="..r.status)
            return
        end
        if not accepted(r) then
            if r.status=="REJECTED_STALE" and p.saw_append and st.plan and not st.blocked
                ...
                dlog("OWN_STALE_AFTER_APPEND uid="..st.uid.." gen="..st.gen.." issue="..p.issue..
        ...
        if p.kind=="MOVE" then
            ...
            if delta>0.05 then
                dlog("OWN_MOVE_CANONICALIZED uid="..st.uid.." gen="..p.gen.." issue="..p.issue.." idx="..p.idx..
        ...
        if p.kind=="MOVE" then
            ...
            dlog("OWN_MOVE_ACK uid="..st.uid.." gen="..st.gen.." issue="..p.issue.." idx="..p.idx.." rev="..st.revision.." model_ms="..string.format("%.0f",now))
            if p.after_attack then
                dlog("P2_MOVE_AFTER_ATTACK_ACK uid="..st.uid.." gen="..st.gen.." issue="..p.issue..
            ...
            dlog("OWN_ATTACK_ACK uid="..st.uid.." gen="..st.gen.." issue="..p.issue.." idx="..p.idx.." target="..p.target_uid.." rev="..st.revision.." model_ms="..string.format("%.0f",now))
            if CONTROLLER_PHASE=="P2B" then
            ...
            if not st.plan[p.idx+1] then
                ...
                dlog("PHASE1_ATTACK_TERMINAL uid="..st.uid.." gen="..st.gen.." model_ms="..string.format("%.0f",now))
                dlog("ATTACK_CURRENT_TAIL uid="..st.uid.." gen="..st.gen.." issue="..p.issue..
...
begin_attack_history=function(st,a,now,source,issue)
    ...
    dlog("ATTACK_ACCEPTED uid="..st.uid.." gen="..st.gen.." issue="..issue.." idx="..st.idx..
...
local function activate(st)
    ...
    dlog("PLAN_ACTIVATED uid="..st.uid.." gen="..st.gen.." actions="..#st.plan..
    ...
    if attack_first then
        ...
        st.owned=false
        dlog("PLAYER_ATTACK_ADOPTED uid="..st.uid.." gen="..st.gen.." serial="..first.serial..
...
local function dispatch(st,index,reason,g,now)
    ...
    if not arm(a.type) then
        if now-st.last_wait>=1000 then
            st.last_wait=now; dlog("WAIT_CALIBRATION uid="..st.uid.." kind="..a.type.." note=natural_player_"..string.lower(a.type).."_required")
        end
        return
    end
    ...
    dlog("DISPATCH_"..a.type.." uid="..st.uid.." gen="..generation.." issue="..issue.." idx="..index..
    ...
    if after_attack then
        dlog("DISPATCH_MOVE_AFTER_ATTACK uid="..st.uid.." gen="..generation.." issue="..issue.." idx="..index..
...
local function advance_attack(st,now)
    ...
    if eligible and not t.previous_eligible then
        dlog("ATTACK_HOLD_BEGIN uid="..st.uid.." gen="..st.gen.." issue="..t.issue.." model_ms="..string.format("%.0f",now)..
    ...
    elseif not eligible and t.previous_eligible then
        dlog("ATTACK_HOLD_SUSPEND uid="..st.uid.." gen="..st.gen.." issue="..t.issue.." model_ms="..string.format("%.0f",now)..
    ...
    if t.eligible_ms>=CFG.attack_hold_ms then
        if not st.plan[st.idx+1] then
            if not t.ready_logged then
                t.ready_logged=true
                dlog("ATTACK_HOLD_READY_NO_TAIL uid="..st.uid.." gen="..st.gen.." issue="..t.issue..
        ...
        if not t.done then
            t.done=true
            dlog("ATTACK_HOLD_DONE uid="..st.uid.." gen="..st.gen.." issue="..t.issue.." idx="..st.idx..
...
local function start()
    ...
    S.started=true
    dlog("READY run="..RUN_ID.." epoch="..epoch.." page_size=64 uid_argument=decimal_string")
...
local function boot()
    ...
    dlog("TEST_BEHAVIOUR_DISABLED profile="..TEST_PROFILE.." artificial_attack_delay_ms=0")
    ...
    if bridge.version()~="0.5.1-per-kind-calibration" then error("WRONG_BRIDGE_VERSION") end
    ...
    log("BRIDGE_OK version=0.5.1-per-kind-calibration")
    local scheduled=false
    local function schedule()
        if scheduled or S.started or S.failed or S.closed then return end
        scheduled=true; dlog("PHASE_CALLBACK Deployed")
    ...
    dlog("LOADED phase="..clean(phase).." run="..RUN_ID)
    if phase=="Deployed" then schedule() else dlog("WAITING_FOR_DEPLOYED") end
end
