...
def main():
    out=(ROOT/'validation_v120');out.mkdir(parents=True,exist_ok=True);L=lua_cmd()
    if not L:raise SystemExit('No Lua host found')
    jobs=[
      ('release_check.txt',[sys.executable,'maintenance_tools/check_release_v120.py']),
    ...
    (out/'checks_result.json').write_text(json.dumps({'variant':'BetterShiftCommand_v1.2.0','status':status,'steps':results},indent=2)+'\n')
