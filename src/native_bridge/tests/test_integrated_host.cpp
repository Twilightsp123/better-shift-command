...
constexpr std::uintptr_t U=0x100000,U2=0x110000,U3=0x120000,U4=0x130000,U5=0x140000,T=0x200000,T2=0x210000,P=0x300000,C=0x400000,A=0x500000,W=0x600000,B=0x700000,V=0x800000,D=0x900000,R=0xa00000,ST=0xb00000;
BridgeHost* h=nullptr;Kind current=Kind::Move;bool queued=false,frame=false;std::uintptr_t handler_root=U;bool call_selection=true,release_in_handler=false;int native_calls=0;Id seq=0;bool reject_publish=false;void* L=reinterpret_cast<void*>(0x1111);
...
void packet_common(Kind k,void* reader){
 ...
 h->order(k,reinterpret_cast<void*>(handler_root),0,reinterpret_cast<void*>(P),queued?1:0);
}
...
struct Fixture{BridgeHost host;Id epoch;
 Fixture(bool authorize=true):host(rd,0x140000000ULL,wr,active,false){memory.clear();for(auto a:{U,U2,U3,U4,U5,T,T2,P,C,A,W,B,V,D,R,ST})memory[a]=std::vector<unsigned char>(0x7000);h=&host;frame=false;call_selection=true;release_in_handler=false;current=Kind::Move;queued=false;handler_root=U;native_calls=0;seq=0;reject_publish=false;
 set(U+0x3ea0,Id(1002));set(U2+0x3ea0,Id(1010));set(U3+0x3ea0,Id(1011));set(U4+0x3ea0,Id(1012));set(U5+0x3ea0,Id(1013));set(T+0x3ea0,Id(1008));set(T2+0x3ea0,Id(1009));set(V+8,std::uintptr_t(D));
 CK(host.set_native_functions({native_order,native_order,allocator,halt}));CK(host.set_adapter_functions({binding,binding,publisher,publisher,begin_writer,finalize,copy,stage,move_handler,attack_handler,select,free_memory}));if(authorize)CK(host.authorize_validated_issue(true));auto e=host.begin("integrated_fixture");CK(e);epoch=e.value;}
 ...
 void observe_recipient(std::uintptr_t root){current=Kind::Move;queued=false;descriptor();host.order(current,reinterpret_cast<void*>(root),0,reinterpret_cast<void*>(P),0);}
 IssueResult issue_recipient(std::uintptr_t root,Kind k=Kind::Move,std::uintptr_t target=T){
  current=k;queued=false;descriptor();set(A,root);set(B+0x5000,Id(0));
  if(k==Kind::Attack){set(C+0x88,target);set(P,target);}
  auto u=get<Id>(root+0x3ea0);auto snap=host.unit_snapshot(u);CK(snap);
  std::optional<std::array<float,3>> dest;
  if(k==Kind::Move)dest=std::array<float,3>{{1.5f,2.0f,3.5f}};
  auto r=host.begin_issue(k,false,u,snap.value.revision,L,dest);if(!r)return r;
  host.binding(k,nullptr,L,0);return host.finish_issue(true);
 }
 void consume_recipient(std::uintptr_t root,Kind k=Kind::Move,std::uintptr_t target=T,float move_delta=0,bool q=false){
  current=k;queued=q;handler_root=root;descriptor();
  if(k==Kind::Attack)set(P,target);else set(P,1.5f+move_delta);
  set(R+8,std::uint8_t(0));set(R+0x10,std::uintptr_t(R+0x1000));
  set(R+0x18,Id(83));set(R+0x1c,Id(0));set(R+0x20,Id(7));
  host.packet_handler(k,reinterpret_cast<void*>(R),nullptr);
 }
...
int main(){int pass=0,fail=0;auto test=[&](const char* name,auto fn){try{fn();++pass;std::cout<<"PASS "<<name<<'\n';}catch(const std::exception& e){++fail;std::cout<<"FAIL "<<name<<" "<<e.what()<<'\n';}};
 ...
 test("two exact physical path witnesses arm only after explicit acknowledgement",[]{Fixture f;f.publish_external(Kind::Move);f.consume();CK(!f.host.status().physical_path_witnesses_ready);f.publish_external(Kind::Attack);f.consume(true);CK(f.host.status().physical_path_witnesses_ready&&f.host.status().verified_issue&&f.host.status().native_issue_authorized);CK(f.host.arm(true)==nullptr);});
 test("default production policy cannot be armed by path samples alone",[]{Fixture f(false);f.publish_external(Kind::Move);f.consume();f.publish_external(Kind::Attack);f.consume();CK(std::string(f.host.arm(true))=="V3_NATIVE_ISSUE_NOT_AUTHORIZED");CK(!f.host.status().verified_issue&&!f.host.status().exact_source);});
 test("runtime-authorized V3 calibration arms with accepted kinds plus any handler",[]{Fixture f;f.calibrate_runtime_shape();auto st=f.host.status();CK(st.native_issue_authorized&&st.v3_issue_armed&&st.v3_issue_calibration_ready&&st.experimental_issue_armed&&st.experimental_calibration_ready&&!st.physical_path_witnesses_ready&&!st.handler_calibration_ready&&st.handler_move_seen==2&&st.handler_attack_seen==0);});
 ...
 test("speculative pending handler ignores unrelated same-kind recipient",[]{Fixture f;f.calibrate_handlers_only();auto i=f.issue();CK(i);handler_root=U2;f.consume(false,false,false,false,true);auto ext=f.events().events.back();auto mid=f.host.status();CK(ext.source==Source::Unknown&&ext.order.recipient.uid==1010&&mid.fatal_errors==0&&mid.gate_fault==Error::Ok);handler_root=U;f.consume(false,false,false,false,true);auto own=f.events().events.back();auto st=f.host.status();CK(own.source==Source::OurController&&own.script_issue_id==i.id&&st.fatal_errors==0&&st.native_packet_mapped>=1);});
 test("V3 owned attack binds at native entry when attack handler is bypassed",[]{Fixture f;f.calibrate_runtime_shape();auto i=f.issue(Kind::Attack);CK(i);current=Kind::Attack;queued=false;descriptor();f.host.order(Kind::Attack,reinterpret_cast<void*>(U),0,reinterpret_cast<void*>(P),0);auto e=f.events().events.back();auto st=f.host.status();CK(e.source==Source::OurController&&e.script_issue_id==i.id&&e.order.target_uid==1008&&st.native_attack_token_bindings==1);});
 ...
 test("unauthorized build cannot arm from handler kinds alone",[]{Fixture f(false);f.publish_external(Kind::Move);f.consume(false,false,false,false,true);f.publish_external(Kind::Attack);f.consume(false,false,false,false,true);auto st=f.host.status();CK(st.handler_calibration_ready&&!st.physical_path_witnesses_ready);CK(std::string(f.host.arm(true))=="V3_NATIVE_ISSUE_NOT_AUTHORIZED");});
 ...
 test("recoverable external capture anomaly does not permanently disarm issuing",[]{
  Fixture f;f.calibrate_runtime_shape();auto before=f.host.status();CK(before.experimental_issue_armed);
  f.host.halt(reinterpret_cast<void*>(0xdeadbeef),1);auto st=f.host.status();
  CK(st.capture_errors==1&&st.fatal_errors==0&&st.gate_fault==Error::Ok&&st.experimental_issue_armed);
  CK(std::string(st.last_recoverable_error)=="HALT_UID_UNREADABLE"&&st.last_recoverable_uid==0);
  CK(f.issue(Kind::Move));
 });
 ...
 test("R4 pending lane no longer batches a multi-unit handoff at four",[]{
  Fixture f;f.calibrate_runtime_shape();for(auto u:{U2,U3,U4,U5})f.observe_recipient(u);
  std::map<std::uintptr_t,Id> ids;for(auto u:{U,U2,U3,U4,U5}){auto i=f.issue_recipient(u);CK(i);ids[u]=i.id;}
  CK(f.host.status().pending_count==5&&f.host.status().pending_limit==32);
  for(auto u:{U3,U5,U,U4,U2}){f.consume_recipient(u);const auto e=f.events().events.back();
   CK(e.source==Source::OurController&&e.script_issue_id==ids[u]&&e.order.recipient.uid==get<Id>(u+0x3ea0));}
  CK(f.host.status().pending_count==0&&f.host.status().fatal_errors==0);
 });
 test("RC6 same recipient cannot stack Move and Attack while other recipient can issue",[]{
  Fixture f;f.calibrate_runtime_shape();f.observe_recipient(U2);CK(f.issue_recipient(U));
  auto blocked=f.issue_recipient(U,Kind::Attack);CK(!blocked&&std::string(blocked.error)=="ISSUE_ALREADY_PENDING");
  CK(f.issue_recipient(U2,Kind::Attack));f.consume_recipient(U2,Kind::Attack);f.consume_recipient(U);
  CK(f.host.status().pending_count==0&&f.host.status().fatal_errors==0);
 });
 test("RC6 unrelated same-kind recipient cannot steal either pending token",[]{
  Fixture f;f.calibrate_runtime_shape();for(auto u:{U2,U3})f.observe_recipient(u);
  auto a=f.issue_recipient(U);auto b=f.issue_recipient(U2);CK(a&&b);
  f.consume_recipient(U3);CK(f.events().events.back().source==Source::Unknown&&f.host.status().pending_count==2);
  f.consume_recipient(U2);CK(f.events().events.back().script_issue_id==b.id);
  f.consume_recipient(U);CK(f.events().events.back().script_issue_id==a.id&&f.host.status().pending_count==0);
 });
 test("RC6 Move fallback requires submitted destination and stale original never executes",[]{
  Fixture f;f.calibrate_runtime_shape();CK(f.issue_recipient(U));int before=native_calls;
  f.consume_recipient(U,Kind::Move,T,100);CK(native_calls==before+1);
  CK(f.events().events.back().source==Source::Unknown&&f.host.status().pending_count==1);
  f.consume_recipient(U);auto e=f.events().events.back();
  CK(e.source==Source::OurController&&e.status==Status::RejectedStale&&native_calls==before+1);
  CK(f.host.status().pending_count==0&&f.host.status().fatal_errors==0);
 });
 test("RC6 two pending attacks resolve by recipient and target not order",[]{
  Fixture f;f.calibrate_runtime_shape();f.observe_recipient(U2);
  auto a=f.issue_recipient(U,Kind::Attack,T);auto b=f.issue_recipient(U2,Kind::Attack,T2);CK(a&&b);
  // Direct native Attack path bypasses the candidate handler, as in the runtime baseline.
  current=Kind::Attack;set(P,std::uintptr_t(T2));f.host.order(current,reinterpret_cast<void*>(U2),0,reinterpret_cast<void*>(P),0);
  CK(f.events().events.back().script_issue_id==b.id&&f.events().events.back().order.target_uid==1009);
  set(P,std::uintptr_t(T));f.host.order(current,reinterpret_cast<void*>(U),0,reinterpret_cast<void*>(P),0);
  CK(f.events().events.back().script_issue_id==a.id&&f.host.status().pending_count==0);
 });
 test("RC6 wrong target cannot claim Attack token; sibling token stays healthy",[]{
  Fixture f;f.calibrate_runtime_shape();f.observe_recipient(U2);
  CK(f.issue_recipient(U,Kind::Attack,T));auto b=f.issue_recipient(U2);CK(b);
  f.consume_recipient(U,Kind::Attack,T2);CK(f.events().events.back().source==Source::Unknown);
  int before=native_calls;f.consume_recipient(U,Kind::Attack,T);
  CK(native_calls==before&&f.events().events.back().status==Status::RejectedStale);
  f.consume_recipient(U2);CK(f.events().events.back().script_issue_id==b.id&&f.host.status().fatal_errors==0);
 });
 test("RC6 queued external order cannot consume pending replacement",[]{
  Fixture f;f.calibrate_runtime_shape();CK(f.issue_recipient(U));
  f.consume_recipient(U,Kind::Move,T,0,true);CK(f.events().events.back().source==Source::Unknown);
  int before=native_calls;f.consume_recipient(U);CK(native_calls==before);
  CK(f.events().events.back().status==Status::RejectedStale&&f.host.status().pending_count==0);
 });
 test("explicit pending cancel releases exact recipient slot",[]{Fixture f;f.calibrate_runtime_shape();auto i=f.issue();CK(i);CK(f.host.status().pending_count==1);CK(f.host.cancel_pending_issue(i.id)==Error::Ok);CK(f.host.status().pending_count==0);CK(f.host.cancel_pending_issue(i.id)==Error::Missing);});
 test("RC6 disarm cancels all pending tokens without native side effects",[]{
  Fixture f;f.calibrate_runtime_shape();f.observe_recipient(U2);
  CK(f.issue_recipient(U));CK(f.issue_recipient(U2));CK(f.host.arm(false)==nullptr);
  int before=native_calls;f.consume_recipient(U2);f.consume_recipient(U);
  CK(native_calls==before&&f.host.status().pending_count==0);
  CK(f.events().events.back().source==Source::OurController);
 });
 test("RC6 rejected metadata and active callback do not evict another token",[]{
  Fixture f;f.calibrate_runtime_shape();f.observe_recipient(U2);CK(f.issue_recipient(U));
  auto snap=f.host.unit_snapshot(1010);CK(f.host.begin_issue(Kind::Move,false,1010,snap.value.revision,L));
  auto reentrant=f.host.begin_issue(Kind::Move,false,1002,0,L);CK(!reentrant);
  CK(!f.host.finish_issue(false));CK(f.host.status().pending_count==1);
  f.consume_recipient(U);CK(f.host.status().pending_count==0);
 });
 test("RC6 epoch shutdown clears all pending identities",[]{
  Fixture f;f.calibrate_runtime_shape();f.observe_recipient(U2);CK(f.issue_recipient(U));CK(f.issue_recipient(U2));
  CK(f.host.end(f.epoch)==Error::Ok&&f.host.status().pending_count==0);
  auto fresh=f.host.begin("next_battle");CK(fresh&&fresh.value!=f.epoch&&f.host.status().pending_count==0);
  CK(!f.host.begin_issue(Kind::Move,false,1002,0,L));
 });
