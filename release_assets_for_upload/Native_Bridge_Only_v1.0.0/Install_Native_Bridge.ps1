...
$receipt = [ordered]@{ version="1.0.0"; game_root=$GameRoot; bridge_only=[bool]($true); installed_at=(Get-Date).ToString("o"); backup_root=$backupRoot; files=@() }
...
if (-not ($true)) { Write-Host "Enable better_shift_command.pack in your WH3 Mod Manager before launching." }
